...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [news,projects,companies,careers,team]=await Promise.all([
    ...
    prisma.teamMember.findMany({where:{published:true},select:{slug:true,updatedAt:true}}),
  ]);
  const staticRoutes=["","/about","/companies","/projects","/impact","/news","/careers","/contact"].map(route=>({url:`${base}${route}`,lastModified:new Date()}));
  return [...staticRoutes, {url:`${base}/team`,lastModified:new Date()},
    ...team.map(item=>({url:`${base}/team/${item.slug}`,lastModified:item.updatedAt})),
